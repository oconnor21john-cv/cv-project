# Java Order Microservices (Spring Boot + Kafka)
...
- Kafka events: `orders.events`, `inventory.events`, `payments.events`
...
Requires Docker running (Testcontainers spins up Postgres + Kafka):
...
Netlify hosts the **React frontend only**. The three Spring Boot services (plus Kafka and Postgres) must be deployed separately on a server platform — [Railway](https://railway.app), [Render](https://render.com), or [Fly.io](https://fly.io) all work well with Docker Compose.
